...
GridFSStorage.prototype._createConnection = function _createConnection() {
  ...
  this.connecting = true;
  MongoClient.connect(url, { useNewUrlParser: true }, options, (err, _db) => {
    let parsedUri;
...
/**
 * Create a writable stream with backwards compatibility with GridFSBucket
...
GridFSStorage.prototype.createStream = function createStream(opts) {
  ...
  if (this._legacy) {
    // `disableMD5` is not supported in GridFSBucket
    settings = {
    ...
    gfs = new GridFSBucket(this.db, opts.id, opts.filename, 'w', settings);
    return gfs.stream();
  }
  settings = {
    id: opts.id,
...
GridFSStorage.prototype._removeFile = function _removeFile(req, file, cb) {
  ...
  if (this._legacy) {
    ...
    GridFSBucket.unlink(this.db, file.id, options, cb);
  } else {
